...
public class E01_Exception {
	...
	/*
	 	...
	 	- 예외가 발생했을때 예외처리가 되어있지 않은 경우 프로그램이 강제종료되지만
	 	  해당 예외에 대한 처리가 되어있는 경우 프로그램이 강제종료되지 않는다
	 	- 예외가 발생하여 해당catch문에 도착할 때 발생한 예외에 대한 정보를 담고있는
	 	  인스턴스가 함께 도착한다 
	 	- 예외 처리의 기본 동작은 e.printStackTrace() 후 강제 종료이다
	...
	public static void main(String[] args) {
		...
		try {
			int size = 10/ 1; // 발생 가능한 예외1
			int[] arr = new int[size];
			System.out.println(arr[-1]); // 발생 가능한 예외2
		}catch (java.lang.ArrayIndexOutOfBoundsException e) {
			System.out.println("인덱스가 잘못된 숫자입니다");
		}catch (java.lang.ArithmeticException e) {
			// 예외가 발생했을 당시의 스택 상황을 출력해주는 메서드 (예외처리 기본 동작1)
			e.printStackTrace();
			// 프로그램을 강제 종료한다(예외처리 기본 동작2)
			System.exit(-1);
		}
