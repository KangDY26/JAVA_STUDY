		...
		/*
		 	# 정수 타입
		 	  - byte	(1byte, 8bit)
		 	  	...
		 	  	byte타입 변수에 넣을 수 있는 정수의 범위는 -128 ~ +127 (256가지 {0포함이다})
		 	  
		 	  - short	(2byte, 16bit)
		 	  
		 	  	2^16(65,536)가지의 값을 표현할 수 있다
		 	  	byte타입 변수에 넣을 수 있는 정수의 범위는 -32,768 ~ +32,767 (65,536가지 {0포함이다})
		 	  	
		 	  - char	(unsigned 2byte)
		 	  
		 	  	2^16(65536) 가지의 값을 표현할 수 있으나 부호가 없다 (양수만 표현한다) char타입 변수에 넣을 수 있는 정수의 범위0 ~ 65535 (65536가지)
		 	  
		 	  - int		(4byte, 32bit)
		 	  	
		 	  	2^32(4,294,967,296)가지의 값을 표현할 수 있다
		 	  	byte타입 변수에 넣을 수 있는 정수의 범위는 -2,147,483,648 ~ +2,147,483,647 (4,294,967,296가지 {0포함이다})
		 	  
		 	  - long	(8byte, 64bit)
		 	  
		 	  	2^64(18,446,744,073,709,551,616)가지의 값을 표현할 수 있다
		 	  	byte타입 변수에 넣을 수 있는 정수의 범위는 -2^63 ~ +2^63-1 (2^64가지 {0포함이다})
		 	...
		 	# 참조형 변수 타입
		 	  ...
		 	  - 문자열
		 	  - 그 외 모든 타입(모든 클래스, 객체지향 관련 타입)

		 */
	public static void main(String[] args) {
		byte max_byth=127,min_byth=-128; 
		...
		char ch4 = 'c';
		char ch5 = 'h';
		하면 된다 굳이 아스키코드 찾아 볼 필요없다
		 */
		
		System.out.println("byth타입에 들어갈 수 있는 가장 큰 값은 " + max_byth + "이고, 가장 작은 값은 " + min_byth + "입니다.");
		System.out.println("short타입에 들어갈 수 있는 가장 큰 값은 " + max_short + "이고, 가장 작은 값은 " + min_short + "입니다.");
		System.out.println("int타입에 들어갈 수 있는 가장 큰 값은 " + max_int + "이고, 가장 작은 값은 " + min_int + "입니다.");
		System.out.println("long타입에 들어갈 수 있는 가장 큰 값은 " + max_long + "이고, 가장 작은 값은 " + min_long + "입니다.");
		
		// 정수 리터럴의 기본 타입은 int
		// 실수 리터럴의 기본 타입은 double
		double d1 = 123.123;
		
		// 8byte 크기의 실수 리터럴을 float타입 변수에 저장할 수 없음으로 뒤에 F를 붙여 float타입 리터럴임을 명시해야 한다
		float f1 = 123.123F;
