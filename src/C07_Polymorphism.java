...
public class C07_Polymorphism {
	/*
	 	...
	 	- 객채가 다양한 형태를 지닐 수 있다는 성질
	 	- 강아지는 강아지이기도 하고 동물이기도 하다
	 	  (강아지는 동물이다(o), 동물은 강아지다(x))
	 	- 아메리카노는 아메리카노이기도 하고 커피이기도 하다
	 	  (아메리카노는 커피이다(o) 커피는 아메리카노인다 (x))
	 	- 자식 클래스 인스턴스는 부모 클래스 타입의 변수애 대입될 수 있다
	 	...
	 	- 자식 클래스 인스턴스가 업캐스팅 된 상태에서 
	 	  오버라이딩된 메서드(자식 클래스에서 마음대로 개조한 메서드)를 사용하는 경우 자식 클래스의 메서드가 실행된다 
	 	
	 	# 다운 캐스팅
	 	
	 	- 부모 타입은 일반적으로 자식타입이 될 수 없다
	 	- 업캐스팅되었던 자식 클래스의 인스턴스만이 다시 원래 타입으로 돌아올 수 있다
	...
	public static void main(String[] args) {
		...
		dog1.pet();
		dog2.pet(); // 오버라이딩된 메서드는 업캐스팅되더라도 자식 클래스의 것으로 동작한다
		...
		System.out.println("wolf1은 강아지 인가요?" + (wolf1 instanceof Dog));
		System.out.println("wolf2은 강아지 인가요?" + (wolf2 instanceof Dog));
