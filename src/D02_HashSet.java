...
public class D02_HashSet {
	...
	public static void main(String[] args) {
		...
		// forEach : 내용을 하나씩 꺼내면서 반복하는 반목 문법
		...
		System.out.println("animals에 lion이 있나요?"
		...
		// addAll(Collection) : 다른 컬랙션의 모든 내용을 더한다
		// removeAll(Collection): 다른 컬랙션에 있는 모든 내용을 제거한다
		// retainAll(Collection): 두 컬랙션에 모두 포함된 내용만 남긴다
		animals.addAll(otherZoo);
		
		System.out.println(animals);
		
		// 컬랙션끼리 다른 컬랙션으로 변환이 자유롭다
		// (생성자로 변환은 지원해준다)
		HashSet<String> set = new HashSet<>(otherZoo);
		System.out.println(set);
		
		ArrayList<String> list = new ArrayList<String>(animals);
		list.add("악어");
		list.add("악어");
		list.add("악어");
