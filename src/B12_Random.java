...
public class B12_Random {
	...
	public static void main(String[] args) {
		...
		// 새로운 랜덤 시드 생성 (생성 시점에서 모든 숫자가 경정된다)
		// 랜덤 시드를 생성하면서 원하는 시드 번호를 전달할 수도 있다
		// 전달하지 않은 경우 현재 시간이 시드 번호가 된다
		...
		for (int i = 0; i < 10; ++i) {
			System.out.println(ran3.nextInt(1000)); // 0~99의 정수
		}
		System.out.println();
		// 원하는 범위의 랜덤 숫자를 생성하려면?
		// (1) bound에 원하는 숫자의 개수를 입력하고
		...
		System.out.println();
		// <연습> 30 ~ 60의 랜덤 숫자 30개 출력하기 
		for (int i = 30 ; i <= 61; ++i) {
			System.out.println(ran3.nextInt(61)); 
		}
		...
		/*
		 	# Math.random()으로 원하는 범위 정수 만들기
		 	
		 	(1) 원하는 숫자의 개수를 곱한다 (bound역할)
		...
		// 20 ~39의 랜덤 정수 만들어보기
		for (int i = 0; i < 10; ++i) {			
			System.out.println((int)(Math.random() * 20) + 20); // 0 ~ 19
		}
