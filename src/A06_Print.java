...
public class A06_Print {
	...
	/*
	 	...
	 	- print format
	 	- 서식을 사용해서 데이터를 출력할 수 있는 함수
	 	- 출력하고 싶은 문자열의 형식을 먼저 지정한 후 값이 들어가야 하는 자리에 데이터를 순서대로 채워서 사용한다
	 	- printf는 줄을 자동으로 바꿔주지 않기 때문에 주의해야한다
	 	
	 	* printf()에 사용할 수 있는 서식 문자의 종류
	 	  
	 	  %d : 해당 자리에 전달한 정수값을 10진수로 출력 (decimal)
	 	  %o : 해당 자리에 전달한 정수값을 8진수로 출력 (octal)
	 	  %x : 해당 자리에 전달할 정수값을 16진수로 출력, 알파벳을 소문자로 출력 (hexa)
	 	  %X : 해당 자리에 전달한 정수값을 16진수로 출력, 알파벳을 대문자로 출력
	 	  %f : 실수
	 	  %s : 문자열
	 	  %c : 문자
	 	  %% : 그냥%를 출력하고 싶은경우 (서식 문자의 Escape)
	 	  
	 	 * 서식 문자 옵션 - 서식 문자의 %와 문자 사이에 넣어서 사용
	 	  
	 	   1. %숫자d : 숫자만큼 자릿수(너비)를 확보하면서 출력한다
	 	   ...
	 	   5. %.숫자f : 실수의 소수점 아래 자리수를 설정할 수 있다
	...
	public static void main(String[] args) {
		...
		System.out.printf("15를 16진수로 표현하면? %x\n", 15);
		System.out.printf("15를 16진수로 표현하면? %X\n", 15);
