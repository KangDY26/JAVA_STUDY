...
public class D11_Regex {
	/*
	 	...
	 	- 검사하고자 하는 문자열에 정규표현식을 적응한 결과를 담고있는 객체
...
//		System.out.println(Pattern.matches("C:[/\\]","C:\\")); X
		...
		System.out.println(Pattern.matches("\\D\\D", "ba"));
		...
		System.out.println(Pattern.matches("\\w{3,}", "abcdef"));
		...
		System.out.println(Pattern.matches("01[0-9]-\\d{4}-\\d{4}", "010-1234-5678"));
		...
		System.out.println(Pattern.matches("01[01679]-?[0-9]{3,4}-?[0-9]{4}","019-234-1234"));
		System.out.println("-----------------");
		// 연습2 : 해당 문자열이 이메일인지 검사할 수 있는 정규표현식을 만들어보세요
		System.out.println(Pattern.matches("\\w+@\\w+.+","java@hamil.co.kr"));
		
		System.out.println(Pattern.matches("\\w+@\\w+.\\w+(\\.\\w+)?","a@b.c"));
		System.out.println(Pattern.matches("\\w+@\\w+.\\w+(\\.\\w+)?","123a123@bcd12.c4.d1"));
		System.out.println("-----------------");
		// 연습3 : 해당 문자열이 IP주소인지 검사 할 수 있는 정규표현식을 만들어보세요(0.0.0.0 ~ 255.255.255.255)
		System.out.println(Pattern.matches("([0-9])|([2][0-5][0-5])|([1][0-9][0-9])|([1-9][0-9])","255"));
		
		System.out.println(Pattern.matches(
				"((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])[.]){3}"
				+ "((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9]))" , "10.245.188.0"));

		String text = "apple/kiwi/banana/mango/kiwi/kiwibird/kiwijuice/goldkiwi/redkiwi/redapple/greenapple";
		...
		while (kiwiMatcher.find()) {
			...
			System.out.println("이번에 find()한 시작 위치: " + kiwiMatcher.start(0));
			System.out.println("이번에 find()한 마지막 위치: " + kiwiMatcher.end(0));
			System.out.println("----------------------");
		}
		
		// 연습: 위의 text에서 apple을 모두 찾아서 kiwi로 교체한 문자열을 생성해보자	

		String replString = text.replace("apple", "kiwi");
