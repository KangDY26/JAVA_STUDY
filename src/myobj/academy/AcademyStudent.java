...
public class AcademyStudent {
	...
	private static NameGenrator nameGenerator = new NameGenrator();
	private static StudentGenrator studentGenrator = new StudentGenrator("EZEN");
	...
	protected AcademyStudent() {
		...
		this.stuNum =  studentGenrator.generate();
	}
	
	public static void printDatail(AcademyStudent stu) {
		System.out.println("-----------------------");
...
//		System.out.println("평균 : " + stu.getAvg());
		System.out.println("등급 : " + stu.getGreade());
		System.out.println("-----------------------");

	}
	
	protected static int genrateRandomScore() {
	...
	// 하지만 세부적인 구현은 모두 달라야한다 -> 상속 받아서 오버라이드 해서 쓰겠다
	...
	public String getGreade() {
		// 현재 인스턴스가 무엇인지에 따라 오버라이딩 된 getAvg()가져다 사용된다
		double avg = getAvg() ;
