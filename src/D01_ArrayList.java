...
public class D01_ArrayList {
	/*
	 	...
	 	- 데이터를 가장 효율적으로 다루기 위해 어떻게 해야 하는가?
	 	  ex) 수 많은 데이터 중 특정 데이터를 찾기 위해서는 어떻게 하는게 가장 효율적일까?
	 	  	  수 많은 데이터들을 순서대로 재배치하는데 가장 효율적인 방법은 무엇일까?
	 	- 세상에는 이미 수 많은 정답들이 알고리즘 형태로 존재하고 있다
	 	
	 	# Java Collections
	 	
	 	- 자료 구조들을 구현해놓은 클래스들의 모음을 자바에서는 컬랙션이라고 부른다
...
//		list.add(4, 999);
		...
		// list.get(index) : 리스트의 해당 번째 요소를 꺼내 반환한다
		// 어떤 타입으로 꺼내야 할지 모르는 경우 Object타입으로 꺼낸다
		System.out.println(list.get(0)); 
		System.out.println(list.get(3)); 
		System.out.println(list.get(4));
		
		// list.size() : 해당 리스트의 크기를 변환한다
		...
		// Object타입으로 꺼내지기 때문에 다운캐스팅을 해야한다
		//System.out.println(list.get(4)+list.get(5)); X
		...
		// 일치하는 값이 여러개면 맨앞에 있는 값 하나만 지운다
		// 						(성공 여부를 반환한다)
		// ※ int의 경우 값으로 삭제하고 싶으면 Integer.valueOf() 메서드를 사용한다
		...
		System.out.println("원래있던 값: " + modified);
		System.out.println(scores);
		...
		// list.sublist(start, end) : 리스트의 일부분을 새로운 리스트로 만들어 반환한다
		// 								(from이상 to 미만)
		...
		// list.addAll(list2) : list1에 list2의 모든 값을 추가한다
		num1.addAll(num2);
		System.out.println(num1);
		
		// list.removeAll(list2) : list1에서 list2에 포함된 모든 값을 삭제한다
		num1.removeAll(num2);
		System.out.println(num1);
