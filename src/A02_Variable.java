...
public class A02_Variable {
	...
	/*
	 	...
	 	- 전기가 통하는 동안 데이터기 계속 유지되는 성질을 가지고 있는 저장 매체
	 	- 현재 진행상황을 보관해놓는 용도로 사용하는 기억 장치
	 	
	 	# 하드드라이브 (보조기억장치)
	 	
	 	- 전기가 통하지 않더라도 데이터를 계속해서 보관해주는 성질을 가지고 있는 저장 매체
	 	- 주로 어떤 프로그램으로 '저장하기' 기능을 수행했을 때 보조기억장치에 해당 상황을 데이터를 보관한다
	...
	// 만약 main() 함수에 틀린점이 있다면 가장 최근에 성공했던 main을 대신 실행한다
	// main 이나 []실수가 많이난다(main 글씨가 틀리거나 []를 안쓰거나)
	public static void main(String[] args) {
		...
		*/ 
		...
		System.out.println("사용자 점수는 " + user_score + "점이고, 컴퓨터 점수는" + com_score + "점 입니다.");
