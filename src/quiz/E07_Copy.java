...
class CopyDirectory {
	...
	private void copyFile(File f) {
		...
		// 복사하려는 파일의 경로를 만든다
		File destFile = new File(destPath, f.getPath().substring(srcPath.length()));
		
		// 복사하려는 경로의 보모 폴더가 아직 없다면 폴더를 생성한다
		File destParFile = destFile.getParentFile();
		if(!destFile.exists()) {
			destParFile.mkdirs();
		}		
	...
	private void copyFolder(File dir) {
		System.out.println(dir + "폴더를 복사함");
		for (File f : dir.listFiles()) {
			...
			System.out.println("add prant : "+ new File(destPath,f.getPath().substring(srcPath.length())));
		}
	}
