...
public class E02_CorrectNumber {
	...
	/*
	 	...
	 	1. 사용자가 중복없는 4개의 숫자를 한 글자씩 입력해야한다
	 	
	 	2. 사용자가 입력한 숫자가 0 ~ 9 사이의 숫자가 아닌경우 예외가 발생해야 한다
	 	
	 	3. 사용자가 입력한 숫자가 이미 입력했던 숫자인 경우 예외가 발생해야 한다
	 	
	 	4. 숫자 4개를 입력하고 나면 입력했던 숫자를 다시한번 하나씩 입력하여 확인해야 한다
	 	
	 	5. 사용자가 입력했던 숫자와 이번에 입력했던 숫자가 다르다면 예외가 발생해야 한다
	 */
	...
	static class GuessMachine{
		...
		private void guess(int num) throws Exception {
			if (cur >= 4) {
				System.out.println("이미 모든 정답을 맞춘상태입니다");
				return;
			}
...
//		} catch (InvalidNumberException e) {
//			System.out.println("잘못된 숫자가 들어가서 중단됨.");
//		} catch (ExistNumberException e) {
//			System.out.println("중복된 숫자가 들어가서 중단됨.");
//		}
