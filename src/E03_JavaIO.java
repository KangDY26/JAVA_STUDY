...
public class E03_JavaIO {
	...
	/*
	 	...
	 	- 데이터가 Stream을 지나가기 위해서는 해당 데이터를 byre타입으로 변환해야 한다
	...
	public static void main(String[] args) {
		...
		try {
			...
			// (저장매체(CD, USB, HDD, SSD)로 데이터를 내보낼 수 있는 통로를 생성한다)
			...
			try {
				...
				// 다 사용한 통로는 반드시 닫아줘야한다 (안닫아주면 메모리 누수 발생)
				
				// ※ 메모리 누수: 프로그램이 오랫동안 켜져있는 경우 차지하는 메모리 용량이 점점 커지는 현상(운영체제에 의해 강제종료 됨)
				out.close();
				
				
			}catch (IOException e) {
				e.printStackTrace();// 통로로 입력하면 도중 발생한 예외
			}
