...
public class C06_Extend {
	/*
	 	...
	 	- 자식 클래스에서 부모 클래스에서 같은 이름을 가지고 있는 자원을 구분하기 위해서 사용한다
...
class Wolf {
	...
	void pet() {
		System.out.printf("늑대(%s, %d세) 쓰다듬다가 손이 물렸습니다.\n", this.name, this.age);
	}
...
class Dog extends Wolf{
	...
	void walk() {
		System.out.printf("강아지(%s, %d세)와 함께 산택했습니다.\n", this.name, this.age);
		walkDesire -= 10;
	}
...
class PurchaseBuilding extends Building{
	...
	// 하지만, 기본 생성자는 생략이 가능하기 때문에 안적은 것처럼 보이는 경우도 있다
	public PurchaseBuilding() {
//		super(); 기본생성자 이기 때문에 생략이 가능하다
	}
